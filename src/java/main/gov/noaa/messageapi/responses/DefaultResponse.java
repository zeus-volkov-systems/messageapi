...
public class DefaultResponse extends BaseResponse implements IResponse {
    ...
    CompletableFuture<IPacket> process(final List<IConnection> connections, final List<IProtocolRecord> recordSets) {
        final List<CompletableFuture<IPacket>> packetFutures = ListUtils.removeAllNulls(connections.stream().map(c -> {
            try {
                return ConnectionUtils.submitRecords(c,
                        recordSets.stream().filter(r -> r.getConnectionId().equals(c.getId())).findFirst().get());
