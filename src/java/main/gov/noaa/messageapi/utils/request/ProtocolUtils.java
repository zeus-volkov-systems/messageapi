...
public class ProtocolUtils {
     ...
     */
    public static Boolean classifierMatch(final Map<String, Object> collectionClassifiers,
            final Map<String, List<Object>> connClassifiers) {
        final List<String> matchKeys = ListUtils.removeAllNulls(collectionClassifiers.keySet().stream()
                .filter(key -> connClassifiers.keySet().contains(key)).collect(Collectors.toList()));
     ...
     * 
     * @param keys                  A set of keys that match both connection and
     *                              collection, each key corresponding to a singular
     *                              or list value of classifier values
     * @param collectionClassifiers A map of the classifiers in the target
     *                              collection
     * @param connClassifiers       A map of classifiers in the comparison
     *                              connection
     * @return returns true if the classifier pair is a match, false otherwise
     */
    public static Boolean classifierValueMatch(final List<String> keys, final Map<String, Object> collectionClassifiers,
            final Map<String, List<Object>> connClassifiers) {
        final List<String> fullMatch = ListUtils.removeAllNulls(keys.stream().filter(key -> {
            final Object classVal = collectionClassifiers.get(key);
